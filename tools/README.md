...
## v4-lint.py — единый CLI с 8 подкомандами
...
| `ar3` | WP-374 | **AR.3 Полнота:** 11 обязательных блоков подраздела + word_count. Дефолтные блоки в `ar_rules.AR3Config`, опциональный YAML-override через `--blocks-config`. Severity: word_count<200 && missing>2 → FAIL; word_count≥200 && missing=0 → PASS; иначе WARN |
| `ar4` | WP-374 | **AR.4 Анти-ИИ-стиль:** blacklist маркеров LLM-стилистики (`tools/data/ar4-blacklist.txt`) минус whitelist (`tools/data/ar4-whitelist.txt`) плюс тройные параллели `не X, не Y, не Z`. Auto-exclude: frontmatter, code blocks, Pack-цитаты, реплики «...». Severity: net≥3 → FAIL, net≥1 → WARN |
| `ar5` | WP-374 | **AR.5 Онтологическая чистота FPF:** для упоминаний entity-типов из glossary (`tools/data/fpf_entity_type_glossary.yaml`) — проверка маркеров различения в окне ±200 chars (Rule A), контекстных слов в абзаце для requires_context entity (Rule B), коллизий с distinguish_from rivals (Rule C, spam-guard через threshold-агрегацию). Suppression через frontmatter `mastery_node: [distinction]` или `## Различение X vs Y`. Severity: PASS/WARN (без FAIL — не блокирующее) |
...
# WP-374 авторские правила (AR.3/AR.4/AR.5)
python3 tools/v4-lint.py ar3 docs/ru/personal-design/1-1-systemic-self-development/s1-culture-as-admission/
python3 tools/v4-lint.py ar4 docs/ru/personal-design/1-1-systemic-self-development/
python3 tools/v4-lint.py ar5 docs/ru/personal-design/1-1-systemic-self-development/ --window-chars 300

# AR-правила: pytest тесты
