...
Подкоманды:
  ...
  pack-drift    — cp.*/bh.* + существование pack_source (PD.FORM/METHOD.NNN) в Pack (Этап 14)
  completeness  — каждый SS в structure-guide имеет контентный файл (WP-322 Ф3.11)
...
# Pack-ID из имени файла формализации: PD.FORM.022-worldview-structure.md → PD.FORM.022
PACK_ID_FROM_FILENAME_RE = re.compile(r"(PD\.[A-Z]+\.\d+)")
PACK_DRIFT_JSON_VERSION = "1.0.0"


def derive_pack_root(pack_path: Path) -> Path | None:
    """Корень пака (`pack/<domain>/`) для рекурсивного индекса Pack-ID.

    PD.FORM.* живут в 02-domain-entities/formalizations/, PD.METHOD.* — в 03-methods/,
    поэтому родителя файла --pack недостаточно: нужен общий корень домена.
    Возвращает первого предка, чей родитель называется `pack`, иначе None.
    """
    try:
    ...
    for ancestor in resolved.parents:
        if ancestor.parent is not None and ancestor.parent.name == "pack":
            return ancestor
    return None


def build_pack_id_index(pack_root: Path) -> set[str]:
    """Множество доступных Pack-ID (PD.<TYPE>.<NNN>) из имён файлов PD.*.md под корнем пака."""
    ids: set[str] = set()
    for p in pack_root.rglob("PD.*.md"):
        m = PACK_ID_FROM_FILENAME_RE.search(p.name)
        if m:
            ids.add(m.group(1))
    return ids


def check_pack_source_existence(sub: Subsection, available_ids: set[str],
                                 findings: list[Finding]) -> None:
    """G2: каждый concept.pack_source (PD.FORM/METHOD/...) существует как формализация в Pack."""
    for concept in sub.concepts:
        src = concept.get("pack_source")
        if not src:
            continue
        if src not in available_ids:
            findings.append(Finding(
                "error", sub.file, sub.line_start,
                f"{sub.subsection_id}: pack_source «{src}» не найден в Pack "
                f"(понятие «{concept.get('name', '?')}») — битый ref или формализация "
                f"переименована/удалена",
            ))


def _emit_pack_drift(findings: list[Finding], args: argparse.Namespace,
                     internal_error: bool = False) -> int:
    """Терминальный вывод pack-drift по двум контрактам (WP-362 Ф7, peer 2026-06-14-44).

    default (PR-gate v4-lint.yml): человекочитаемые findings, exit 1 при error-findings.
    --json (pack-drift-watcher.yml): JSON {drift_count, warning_count, items[]} в stdout,
      findings в stderr, exit 0 при штатной отработке (дрейф закодирован в JSON),
      exit 2 при сбое самого инструмента (internal_error) — чтобы watcher не зеленел молча
      (различение «инструмент нашёл дрейф» ≠ «инструмент сломался»).
    """
    ...
    print(f"\n[pack-drift] errors={len(errors)} warnings={len(warns)}", file=sys.stderr)

    if internal_error:
        payload = {
            "tool": "v4-lint pack-drift",
            "version": PACK_DRIFT_JSON_VERSION,
            "error": "; ".join(f.message for f in findings) or "internal error",
        }
    ...
    payload = {
        "tool": "v4-lint pack-drift",
        "version": PACK_DRIFT_JSON_VERSION,
        "drift_count": len(errors),       # только error-level → триггер issue
        "warning_count": len(warns),      # advisory, issue не создаёт
        "items": items,
    }
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    return 0
...
def cmd_pack_drift(args: argparse.Namespace) -> int:
    ...
    files, findings = collect_structure_files(targets)
    if not files:
        return _emit_pack_drift(findings, args)
    ...
    if not args.pack:
        ...
        return _emit_pack_drift(findings, args, internal_error=True)

    pack_path = Path(args.pack)
    if not pack_path.exists():
        findings.append(Finding(
            "error", pack_path, None, f"Pack-файл не найден: {pack_path}",
        ))
        return _emit_pack_drift(findings, args, internal_error=True)
    ...
    # G2: индекс доступных Pack-ID для existence-check pack_source.
    # default pack-root выводится из --pack (PD.METHOD живёт не рядом с PD.FORM, нужен корень домена).
    pack_root: Path | None = (
        Path(args.pack_root) if getattr(args, "pack_root", None) else derive_pack_root(pack_path)
    )
    available_ids: set[str] | None = None
    if pack_root and pack_root.exists():
        available_ids = build_pack_id_index(pack_root)
        if not available_ids:
            findings.append(Finding(
                "warning", pack_root, None,
                "pack-root не содержит PD.*.md — G2 (existence pack_source) пропущен",
            ))
            available_ids = None
    else:
        findings.append(Finding(
            "warning", Path("."), None,
            "pack-root недоступен — G2 (existence pack_source) пропущен (downgrade error→skip)",
        ))
        ...
        for sec in sections:
            for sub in sec.subsections:
                ...
                if available_ids is not None:
                    check_pack_source_existence(sub, available_ids, findings)
...
# ============================================================================
# Subcommand: completeness (WP-322 Ф3.11)
# ============================================================================

def cmd_completeness(args: argparse.Namespace) -> int:
    """COMPLETENESS: каждый SS в structure-guide имеет контентный файл.

    Читает structure-guide-N.md → собирает объявленные subsection_id.
    Сканирует --content-dir рекурсивно → ищет subsection_id в frontmatter.
    MISSING (error): объявлен, но файла нет.
    ORPHAN  (warn):  файл есть, но не объявлен — только с --warn-orphans.

    Почему фронтматтер, а не имя файла: соглашение об именах не зафиксировано
    в контракте — batch-skeletons.py генерирует S.NN.md, но normalize-script
    использовал суффиксы -concepts/-exercises. Чтение frontmatter даёт
    инвариантный ответ независимо от файловой конвенции.
    """
    findings: list[Finding] = []

    # 1. Collect declared IDs from structure-guides
    structure_targets = [Path(p) for p in args.paths]
    files, collect_findings = collect_structure_files(structure_targets)
    findings.extend(collect_findings)

    declared: dict[str, Path] = {}  # subsection_id → source structure-guide file
    for f in files:
        sections, parse_findings = parse_structure_file(f)
        findings.extend(parse_findings)
        for sec in sections:
            for sub in sec.subsections:
                if sub.subsection_id:
                    declared[sub.subsection_id] = f

    # Apply optional guide/section filters
    if getattr(args, "guide", None):
        guide_prefix = f"PD.GUIDE.{args.guide}."
        declared = {k: v for k, v in declared.items() if k.startswith(guide_prefix)}
    if getattr(args, "section", None):
        section_mid = f".S{args.section}."
        declared = {k: v for k, v in declared.items() if section_mid in k}

    if not declared and not findings:
        findings.append(Finding("error", Path("."), None,
                                "COMPLETENESS: structure-guide не содержит ни одного subsection_id"))
        return report(findings, label="completeness")

    # 2. Scan content-dir for files that advertise a subsection_id in frontmatter
    content_dir = Path(args.content_dir)
    if not content_dir.exists():
        findings.append(Finding("error", content_dir, None,
                                f"COMPLETENESS: content-dir не существует: {content_dir}"))
        return report(findings, label="completeness")

    found: dict[str, Path] = {}  # subsection_id → content file path
    for md_path in sorted(content_dir.rglob("*.md")):
        sub, _ = parse_single_subsection(md_path)
        if sub and sub.subsection_id:
            if sub.subsection_id in found:
                findings.append(Finding(
                    "warning", md_path, None,
                    f"COMPLETENESS-DUP: {sub.subsection_id} встречается в нескольких файлах "
                    f"(первый: {found[sub.subsection_id]}, дубль: {md_path})",
                ))
            else:
                found[sub.subsection_id] = md_path

    # 3. Compare declared vs found
    declared_ids = set(declared)
    found_ids = set(found)

    missing = sorted(declared_ids - found_ids)
    orphaned = sorted(found_ids - declared_ids)

    for ss_id in missing:
        findings.append(Finding(
            "error", declared[ss_id], None,
            f"COMPLETENESS-MISSING: {ss_id} — объявлен в structure-guide, нет в {content_dir}",
        ))

    if getattr(args, "warn_orphans", False):
        for ss_id in orphaned:
            findings.append(Finding(
                "warning", found[ss_id], None,
                f"COMPLETENESS-ORPHAN: {ss_id} — файл есть, но не объявлен в structure-guide",
            ))

    total_declared = len(declared_ids)
    total_matched = len(declared_ids & found_ids)
    print(
        f"[completeness] объявлено={total_declared} совпало={total_matched} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_drift.add_argument("--pack-root",
                         help="Корень пака для existence-check pack_source "
                              "(default: выводится из --pack)")
    p_drift.add_argument("--json", action="store_true",
                         help="JSON-вывод {drift_count, warning_count, items[]} для watcher "
                              "(exit 0 при отработке, 2 при сбое инструмента)")
    ...
    # WP-322 Ф3.11: completeness — spec vs content cross-check
    p_compl = sub.add_parser(
        "completeness",
        help="Полнота реализации: каждый SS в structure-guide имеет контентный файл",
        description=(
            "Сравнивает объявленные подразделы (structure-guide-N.md) с фактическими\n"
            "файлами в --content-dir (обычно aisystant/docs/docs/ru/personal-design/).\n\n"
            "MISSING (error): subsection_id в structure-guide, но файла нет в content-dir.\n"
            "ORPHAN  (warn):  файл существует, но не объявлен (только с --warn-orphans).\n\n"
            "Определение принадлежности по frontmatter subsection_id, а не по имени файла,\n"
            "что делает проверку инвариантной к соглашению об именах."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Примеры:\n"
            "  # Полная проверка всех 4 руководств:\n"
            "  v4-lint.py completeness specs/v4-reference/ \\\n"
            "      --content-dir ../docs/docs/ru/personal-design/\n\n"
            "  # Только руководство 1:\n"
            "  v4-lint.py completeness specs/v4-reference/ \\\n"
            "      --content-dir ../docs/docs/ru/personal-design/ --guide 1\n\n"
            "  # Раздел 6 руководства 1 + предупреждение об orphans:\n"
            "  v4-lint.py completeness specs/v4-reference/ \\\n"
            "      --content-dir ../docs/docs/ru/personal-design/ --guide 1 --section 6 --warn-orphans\n"
        ),
    )
    p_compl.add_argument("paths", nargs="+",
                         help="Файлы или директория с structure-guide-N.md")
    p_compl.add_argument(
        "--content-dir", required=True,
        help="Путь к директории с контентными файлами (aisystant/docs/docs/ru/personal-design/)",
    )
    p_compl.add_argument(
        "--guide", type=int, choices=[1, 2, 3, 4],
        help="Ограничить проверку одним руководством (1–4)",
    )
    p_compl.add_argument(
        "--section", type=int,
        help="Ограничить проверку одним разделом (1–10)",
    )
    p_compl.add_argument(
        "--warn-orphans", action="store_true",
